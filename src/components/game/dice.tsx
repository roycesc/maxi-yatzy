...
const Dice: React.FC<DiceProps> = ({ 
  ...
  // Generate dots for dice face - smaller dots for mobile
  const renderDots = () => {
    switch(value) {
      case 1:
      ...
      case 2:
        return <div className="absolute inset-0 grid grid-cols-2 p-1">
      ...
      case 3:
        return <div className="absolute inset-0 grid grid-cols-3 grid-rows-3 p-1">
      ...
      case 4:
        return <div className="absolute inset-0 grid grid-cols-2 grid-rows-2 p-1">
          <div className="flex items-start justify-start">
            <div className="w-1.5 h-1.5 rounded-full bg-amber-900"></div>
          </div>
  ...
  return (
    <motion.div
      className={cn(
        "w-10 h-10 relative rounded-md shadow-sm cursor-pointer transition-colors",
        isHeld ? "bg-amber-400 border border-amber-600" : "bg-white",
